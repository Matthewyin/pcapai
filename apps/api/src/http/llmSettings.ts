...
const currentManagedKeys = ["PCAPAI_LLM_BASE_URL", "PCAPAI_LLM_MODEL", "PCAPAI_LLM_API_KEY", "PCAPAI_LLM_ACTIVE_PROFILE"];
const profileKeyPattern = /^PCAPAI_LLM_PROFILE_(.+)_(NAME|BASE_URL|MODEL|API_KEY)$/;
...
function writeEnvValues(removeKey: (key: string) => boolean, values: Map<string, string>) {
  ...
  const managed = [...values.entries()].map(([key, value]) => `${key}=${formatEnvValue(value)}`);
...
export function saveLlmSettings(input: { baseURL: string; model: string; apiKey?: string }) {
  ...
  } else if (nextValues.get("PCAPAI_LLM_API_KEY")) {
    nextValues.set(keys.apiKey, nextValues.get("PCAPAI_LLM_API_KEY") || "");
  }
  ...
  updateLlmConfig({
    ...
    model: input.model,
    apiKey: nextValues.get("PCAPAI_LLM_API_KEY") || ""
  });
...
export function saveLlmProfile(input: { profileId?: string; name: string; baseURL: string; model: string; apiKey?: string }) {
  ...
  const nextApiKey = input.apiKey && input.apiKey.length > 0 ? input.apiKey : current.get(keys.apiKey) || "";
  if (nextApiKey) {
  ...
  updateLlmConfig({ baseURL: input.baseURL, model: input.model, apiKey: nextApiKey });
...
export function activateLlmProfile(profileId: string) {
  ...
  updateLlmConfig({ baseURL, model, apiKey });
  return getLlmSettings();
}
