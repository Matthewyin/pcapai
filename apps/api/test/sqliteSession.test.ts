...
let sessions: SqliteSession[] = [];
function openSession(deps: ConstructorParameters<typeof SqliteSession>[0]) {
  const session = new SqliteSession(deps);
...
test("SqliteSession: getSessionId 返回构造时传入或生成的 id", async () => {
  const s = openSession({ baseDir: tempDir, sessionId: "case-abc-123" });
  assert.equal(await s.getSessionId(), "case-abc-123");
  const s2 = openSession({ baseDir: tempDir });
  const id = await s2.getSessionId();
  assert.ok(typeof id === "string" && id.length > 0);
});

test("SqliteSession: addItems + getItems 往返", async () => {
  const s = openSession({ baseDir: tempDir, sessionId: "rt-1" });
...
test("SqliteSession: getItems(limit) 返回最近 N 条且按时间正序", async () => {
  const s = openSession({ baseDir: tempDir, sessionId: "rt-2" });
  await s.clearSession();
  await s.addItems([
...
test("SqliteSession: popItem 删除并返回最后一条", async () => {
  const s = openSession({ baseDir: tempDir, sessionId: "rt-3" });
  await s.clearSession();
  await s.addItems([
...
test("SqliteSession: popItem 空时返回 undefined", async () => {
  const s = openSession({ baseDir: tempDir, sessionId: "rt-4" });
  await s.clearSession();
  const popped = await s.popItem();
...
test("SqliteSession: clearSession 清空", async () => {
  const s = openSession({ baseDir: tempDir, sessionId: "rt-5" });
...
test("SqliteSession: itemCount 反映条目数", async () => {
  const s = openSession({ baseDir: tempDir, sessionId: "rt-6" });
  await s.clearSession();
...
test("SqliteSession: replaceAllWith 用摘要替换全部历史", async () => {
  const s = openSession({ baseDir: tempDir, sessionId: "rt-7" });
  await s.addItems([
...
test("SqliteSession: 持久化到磁盘文件", async () => {
  const s1 = openSession({ baseDir: tempDir, sessionId: "persist-1" });
  ...
  const s2 = openSession({ baseDir: tempDir, sessionId: "persist-1" });
...
test("SqliteSession: sessionId 含特殊字符被 sanitize（防路径穿越）", () => {
  const s = openSession({ baseDir: tempDir, sessionId: "../evil/path" });
